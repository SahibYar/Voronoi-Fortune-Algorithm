...
{
private:
	vector<shared_ptr<VEvent>> InnerList;
public:
	BinaryPriorityQueue()
	{
		InnerList.empty();
	...
	//{
	//	if (Copy) InnerList = Core.Clone() as ArrayList;
	//	else InnerList = Core;
	//	Comparer = Comp;
	//}
	...
	//protected virtual int OnCompare(int i, int j) { return Comparer.Compare(InnerList[i], InnerList[j]); }
	...
	//		p1 = 2 * p + 1;
	//		p2 = 2 * p + 2;
	//		if (InnerList.Count > p1 && OnCompare(p, p1) > 0) // left small
	//			p = p1;
